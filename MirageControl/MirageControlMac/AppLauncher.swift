...
final class AppLauncher {
    ...
    func launch(bundleID: String) async -> Bool {
        guard let url = NSWorkspace.shared.urlForApplication(withBundleIdentifier: bundleID) else {
            #if DEBUG
            print("MirageControl: ⚠️ No app URL for bundleID \(bundleID)")
            #endif
            return false
        }
        ...
        do {
            ...
            #if DEBUG
            print("MirageControl: ⚠️ Failed to open \(bundleID): \(error)")
            #endif
            return false
        }
    }
    ...
    func activateAndWait(
        ...
        #if DEBUG
        print("MirageControl: ⚠️ \(bundleID) never became frontmost within \(timeout)s; sending shortcut anyway")
        #endif
    }
}

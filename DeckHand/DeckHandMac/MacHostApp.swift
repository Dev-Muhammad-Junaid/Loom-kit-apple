    ...
    init() {
        ...
        do {
            built = try LoomContainer(
                for: LoomContainerConfiguration(
                    ...
                    // `.cloudKitOwn` source (see DeckHandCloud).
            ...
            DeckHandLog.app.fault("LoomContainer init failed: \(error.localizedDescription, privacy: .public)")
            built = nil
        ...
        Task {
            ...
            do {
                ...
                DeckHandLog.app.info("LoomContext started")
                ...
                if DeckHandCloud.remoteSignalingConfiguration != nil {
                    do {
                        let deviceID = LoomSharedDeviceID.getOrCreate(suiteName: "DeckHandLoomStore")
                        let sessionID = "deckhand-\(deviceID.uuidString)"
                        try await context.publishRemoteReachability(sessionID: sessionID)
                        DeckHandLog.app.info("Published remote reachability")
                    } catch {
                        // Remote joins unavailable; local operation unaffected.
                        DeckHandLog.app.error("Remote reachability publish failed: \(error.localizedDescription, privacy: .public)")
                    }
                }
            } catch {
                DeckHandLog.app.fault("LoomContext start failed: \(error.localizedDescription, privacy: .public)")
                await MainActor.run {
            ...
            for await handle in context.incomingConnections {
                ...
                DeckHandLog.connection.info("Incoming connection \(id, privacy: .public) from \(peer.name, privacy: .public)")
            ...
            DeckHandLog.connection.fault("incomingConnections stream ended — host will no longer accept new connections")
        }
...
struct MacHostApp: App {
    // The menu-bar item is created and owned by DeckHandAppDelegate via
    // NSStatusItem rather than SwiftUI's MenuBarExtra, so the item's
    // behavior, visibility, and panel sizing stay under our control.
    @NSApplicationDelegateAdaptor(DeckHandAppDelegate.self) private var appDelegate

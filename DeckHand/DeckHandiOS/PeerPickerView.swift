...
struct PeerPickerView: View {
    ...
    var body: some View {
        ZStack {
            // Hero sunset shader background — see `DeckHandTheme.heroSunsetBackground()`
            DeckHandTheme.heroSunsetBackground()
                .ignoresSafeArea()
            ...
            VStack(spacing: 0) {
                ...
                if let error = errorMessage {
                    Text(error)
                        ...
                        .foregroundStyle(DeckHandTheme.errorBannerLabel(colorScheme))
                        .padding(.horizontal, 20)
                        .padding(.vertical, 10)
                        .background(
                            DeckHandTheme.errorBannerFill(colorScheme)
                                .clipShape(RoundedRectangle(cornerRadius: DeckHandTheme.Radius.sm, style: .continuous))
                        )
    ...
    private var header: some View {
        VStack(spacing: 16) {
            ...
            RoseThreeLoaderView(size: 108, color: DeckHandTheme.violet)
    ...
    private var emptyState: some View {
        DeckHandLoadingStateView(title: "Searching for Macs…")
    }
                    ...
                    HStack(spacing: 6) {
                        ...
                        if peer.isSameICloudDevice {
                            Label("My Mac", systemImage: "icloud.fill")
                                ...
                                .padding(.vertical, 2)
                                .background(
                                    Capsule().fill(DeckHandTheme.violet.opacity(0.12))
                                )
                        }
                    }
                    HStack(spacing: 5) {
                        Circle()
                            .fill(peer.isNearby ? DeckHandTheme.success : Color.orange)

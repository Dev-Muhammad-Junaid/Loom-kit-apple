...
struct QuickActionsBar: View {
    ...
    var body: some View {
        ...
        .clipShape(RoundedRectangle(cornerRadius: DeckHandTheme.Radius.md, style: .continuous))
        .overlay(
            RoundedRectangle(cornerRadius: DeckHandTheme.Radius.md, style: .continuous)
                .strokeBorder(DeckHandTheme.tabContainerBorder(colorScheme), lineWidth: 1)
        )
    ...
    private var barBackground: some View {
        DeckHandTheme.tabContainerFill(colorScheme)
    }
}
...
struct KeyboardFAB: View {
    ...
    var body: some View {
        FABButton(
            ...
            isActive: isActive,
            tint: DeckHandTheme.FAB.keyboardTint,
...
struct NumericFAB: View {
    ...
    var body: some View {
        FABButton(
            ...
            isActive: isActive,
            tint: DeckHandTheme.FAB.numericTint,
...
private struct TextFieldChip: View {
    ...
    private var background: Color {
        action.isPrimary ? DeckHandTheme.violet : DeckHandTheme.subtleWellFill(colorScheme)
    }
}
...
private struct NumpadChip: View {
    ...
    private var background: Color {
        key.isPrimary ? DeckHandTheme.violet : DeckHandTheme.subtleWellFill(colorScheme)
    }
}
...
private struct DialogButtonChip: View {
    ...
    private var background: Color {
        if button.isDefault { return DeckHandTheme.violet }
        if button.isCancel  { return Color.clear }
        return DeckHandTheme.subtleWellFill(colorScheme)
    }

    private var stroke: Color {
        if button.isDefault { return DeckHandTheme.violet.opacity(0.8) }
